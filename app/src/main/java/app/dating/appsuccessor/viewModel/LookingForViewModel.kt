...
class LookingForViewModel : ViewModel() {
    ...
    private val _updateLookingForSuccess = MutableLiveData<Boolean>()
    val updateLookingForSuccess: LiveData<Boolean>
        get() = _updateLookingForSuccess

    fun updateUserLookingFor(lookingFor: String) {
        ...
        currentUserRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                val updatedUserModel = currentUserModel?.copy(lookingFor = lookingFor)

                currentUserRef.setValue(updatedUserModel).addOnCompleteListener {
                    if (it.isSuccessful) {
                        //  updated successfully
                        _updateLookingForSuccess.value = true
                    } else {
                        // Error updating
                        _updateLookingForSuccess.value = false
                    }
                }
            }

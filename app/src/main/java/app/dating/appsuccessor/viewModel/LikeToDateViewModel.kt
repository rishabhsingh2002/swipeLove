...
class LikeToDateViewModel : ViewModel() {

    private val auth by lazy { FirebaseAuth.getInstance() }
    private val database by lazy { FirebaseDatabase.getInstance() }

    private val _updateLikeToDateSuccess = MutableLiveData<Boolean>()
    val updateLikeToDateSuccess: LiveData<Boolean>
        get() = _updateLikeToDateSuccess

    fun updateUserLikeToDate(likeToDate: String) {
        ...
        currentUserRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                val updatedUserModel = currentUserModel?.copy(likeToDate = likeToDate)

                currentUserRef.setValue(updatedUserModel).addOnCompleteListener {
                    if (it.isSuccessful) {
                        //  updated successfully
                        _updateLikeToDateSuccess.value = true
                    } else {
                        // Error updating
                        _updateLikeToDateSuccess.value = false
                    }
                }
            }
